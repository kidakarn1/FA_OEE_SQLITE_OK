...
Public Class List_Emp
    ...
    ' Employee photographs are optional.  Keep the existing no_user.jpg
    ' fallback, but do not allow a missing fallback asset to abort loading
    ' the selected employees.
    Private Function LoadOptionalEmployeeImage(client As WebClient, url As String, Optional fallbackUrl As String = "") As Bitmap
        Try
...
#If DEBUG Then
            Debug.WriteLine("Employee image unavailable: " & url & " | " & ex.Status.ToString())
#End If
        Catch ex As ArgumentException
#If DEBUG Then
            Debug.WriteLine("Employee image is invalid: " & url)
#End If
        Catch ex As OutOfMemoryException
#If DEBUG Then
            Debug.WriteLine("Employee image could not be decoded: " & url)
#End If
        End Try
        If fallbackUrl <> "" AndAlso Not String.Equals(url, fallbackUrl, StringComparison.OrdinalIgnoreCase) Then
            Return LoadOptionalEmployeeImage(client, fallbackUrl)
        End If
        Return Nothing
    End Function
        ...
        For i = 0 To temp_co_emp - 1
            If i = 0 Then
                ...
                Try
                    ...
                    Dim tImage As Bitmap = LoadOptionalEmployeeImage(tclient, url, "http://" & Backoffice_model.svApi & "/tbkk_shopfloor_sys/asset/img_emp/no_user.jpg")
                ...
                Catch ex As Exception
                    Dim emp_cd As String = ListView1.Items(i).Text
                    Dim tImage As Bitmap = LoadOptionalEmployeeImage(tclient, "http://" & Backoffice_model.svApi & "/tbkk_shopfloor_sys/asset/img_emp/no_user.jpg")
                    Backoffice_model.img_user1 = tImage
                    MainFrm.PictureBox2.Image = tImage
                    MainFrm.lb_emp1.Visible = True
                    MainFrm.lb_emp1.Text = emp_cd
            ...
            ElseIf i = 5 Then
                ...
                Else
                    ...
                    Dim tImage As Bitmap = LoadOptionalEmployeeImage(tclient, url)
                    MainFrm.PictureBox7.Image = tImage
                    MainFrm.lb_emp6.Visible = True
                    MainFrm.lb_emp6.Text = emp_cd
                    Backoffice_model.img_user6 = tImage
