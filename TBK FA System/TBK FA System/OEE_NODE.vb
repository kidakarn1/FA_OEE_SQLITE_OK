...
Public Class OEE_NODE
    Public Shared Function OEE_LOAD_MSTOEE(line_cd As String)
        Try
            ...
            Dim jsonString = api.Load_dataOEE("http://" & Backoffice_model.svOEE & "/api/dataGetmstOEE?line_cd=" & line_cd)
    ...
    Public Shared Function OEE_LOAD_MSTOEEColor(line_cd As String)
        Try
            ...
            Dim jsonString = api.Load_dataOEE("http://" & Backoffice_model.svOEE & "/api/dataGetOEEColor?line_cd=" & line_cd)
            Dim dcResultdata As Object = New JavaScriptSerializer().Deserialize(Of List(Of Object))(jsonString)
            Dim i As Integer = 1
            load_show_OEE.Close()
            Return dcResultdata
        Catch ex As Exception
            ' The color endpoint is unavailable or returned an invalid response.
            ' Do not leave the production screen behind an OEE loading overlay.
            load_show_OEE.HideIfOpen()
            MessageBox.Show("OEE service is unavailable. OEE color criteria could not be loaded.",
                            "OEE Service",
    ...
    Public Shared Function OEE_GET_NEW_TARGET(st_shift As String, end_shift As String, std_ct As String, shift As String)
        Try
            ...
            ' Dim TarGet = api.Load_dataOEE("http://" & Backoffice_model.svApi & "/API_NEW_FA/index.php/GET_OEE/NEW_GET_TARGET?st_shift=" & st_shift & "&end_shift=" & end_shift & "&std_ct=" & std_ct)
            ...
            Dim jsonString = api.Load_dataOEE("http://" & Backoffice_model.svOEE & "/api/dataGettarget?st_shift=" & st_shift & "&end_shift=" & end_shift & "&std_ct=" & std_ct & "&date_start=" & convertDateStart & "&date_end=" & date_end)
    ...
    Public Shared Function OEE_GET_NEW_TARGET_PERCEN(st_shift As String, end_shift As String, std_ct As String, shift As String, line_cd As String)
        Try
            ...
            Dim jsonString = api.Load_dataOEE("http://" & Backoffice_model.svOEE & "/api/dataGettargetPercen?st_shift=" & st_shift & "&end_shift=" & end_shift & "&std_ct=" & std_ct & "&date_start=" & convertDateStart & "&date_end=" & date_end & "&line_cd=" & line_cd)
            Dim jsSerializer As New JavaScriptSerializer()
            ' Deserialize the JSON string to a Dictionary
            Dim data As Dictionary(Of String, Object) = jsSerializer.Deserialize(Of Dictionary(Of String, Object))(jsonString)
            ' Access the value
    ...
    Public Shared Function OEE_GET_Hour(shift As String)
        Try
            ...
            ' Dim TarGet = api.Load_dataOEE("http://" & Backoffice_model.svApi & "/API_NEW_FA/index.php/GET_OEE/NEW_GET_TARGET?st_shift=" & st_shift & "&end_shift=" & end_shift & "&std_ct=" & std_ct)
            '   ''Console.WriteLine("http://" & Backoffice_model.svApi & "/API_NEW_FA/index.php/GET_OEE/NEW_GET_TARGET?st_shift=" & st_shift & "&end_shift=" & end_shift & "&std_ct=" & std_ct)
            Dim jsonString = api.Load_dataOEE("http://" & Backoffice_model.svOEE & "/api/dataGetHour?shift=" & shift)
            Dim jsSerializer As New JavaScriptSerializer()
            ' Deserialize the JSON string to a Dictionary
            Dim data As Dictionary(Of String, Object) = jsSerializer.Deserialize(Of Dictionary(Of String, Object))(jsonString)
            ' Access the value
    ...
    Public Shared Function OEE_getProduction_actual_detailByHour(line_cd As String, minSwitchModel As Integer, start_date As String, partNo As String)
        Try
            ...
            Dim jsonString = api.Load_dataOEE("http://" & Backoffice_model.svOEE & "/api/dataDetailByHouse?line_cd=" & line_cd & "&date_crr=" & convertDateCrr & "&time_crr=" & time & "&convertnewDateMinutes=" & convertnewDateMinutes & "&partNo=" & partNo)
            Dim jsSerializer As New JavaScriptSerializer()
            ' Deserialize the JSON string to a Dictionary
            Dim data As Dictionary(Of String, Object) = jsSerializer.Deserialize(Of Dictionary(Of String, Object))(jsonString)
            ' Access the value
    ...
    Public Shared Function OEE_getProduction_actual_detailByShift(line_cd As String, shift As String)
        Try
            ...
            Dim jsonString = api.Load_dataOEE("http://" & Backoffice_model.svOEE & "/api/dataDetailByShift?line_cd=" & line_cd & "&date_start=" & convertDateStart & "&date_end=" & convertDateEnd & "&st_shift=" & st_time & "&end_shift=" & end_time)
            Dim jsSerializer As New JavaScriptSerializer()
            ' Deserialize the JSON string to a Dictionary
            Dim data As Dictionary(Of String, Object) = jsSerializer.Deserialize(Of Dictionary(Of String, Object))(jsonString)
            ' Access the value
    ...
    Public Shared Function GetDataProgressbarA(st_shift As String, end_shift As String, line_cd As String, dateTimeswmodel As String, statusSwitchModel As String, IsOnlyone As String)
        Try
            ...
            Dim jsonString = api.Load_dataOEE("http://" & Backoffice_model.svOEE & "/api/dataProgressA?st_shift=" & st_shift & "&end_shift=" & end_shift & "&line_cd=" & line_cd & "&date_start=" & convertDateStart & "&date_Crr=" & convertDateCrr & "&TimeCrr=" & time & "&dateTimeswmodel=" & dateTimeswmodel & "&statusSwitchModel=" & statusSwitchModel & "&IsOnlyone=" & IsOnlyone)
            Dim jsSerializer As New JavaScriptSerializer()
            ' Deserialize the JSON string to a Dictionary
            Dim data As Dictionary(Of String, Object) = jsSerializer.Deserialize(Of Dictionary(Of String, Object))(jsonString)
            ' Access the value
    ...
    Public Shared Function OEE_GetLossByHouseP1(line_cd As String)
        Try
            ...
            Dim jsonString = api.Load_dataOEE("http://" & Backoffice_model.svOEE & "/api/dataGetlossbyhouse?line_cd=" & line_cd & "&date_start=" & date_st & "&date_end=" & date_end & "&time_crr=" & time & "&convertnewDateMinutes=" & convertnewDateMinutes)
            ' Deserialize the JSON string to a Dictionary
            Dim jsSerializer As New JavaScriptSerializer()
    ...
    Public Shared Function OEE_GET_Data_LOSS(line_cd As String, lot_no As String, shift As String, dateStart As String, dateTimeswModel As String, statusSwitchModel As String, IsOnlyone As String)
        Try
            ...
            Dim jsonString = api.Load_dataOEE("http://" & Backoffice_model.svOEE & "/api/dataGetDataAvailabillty?line_cd=" & line_cd & "&lot_no=" & lot_no & "&shift=" & shift & "&dateStart=" & convertDateStart & "&dateEnd=" & date_end & "&st_shift=" & st_time & "&end_shift=" & time & "&dateTimeswModel=" & dateTimeswModel & "&statusSwitchModel=" & statusSwitchModel & "&IsOnlyone=" & IsOnlyone)
    ...
    Public Shared Function OEE_GET_Data_AccTarget(st_shift As String, std_ct As String)
        Try
            ...
            Dim jsonString = api.Load_dataOEE("http://" & Backoffice_model.svOEE & "/api/datagetAccTarget?st_shift=" & st_shift & "&std_ct=" & std_ct & "&dateStart=" & convertDateStart & "&dateEnd=" & date_end & "&end_shift=" & time)
    ...
    Public Shared Function OEE_getSpeedLoss(NG As String, Good As String, Timeshift As String, std_cd As String)
        Try
            ...
            Dim jsonString = api.Load_dataOEE("http://" & Backoffice_model.svApi & "/API_NEW_FA/index.php/GET_OEE/getSpeedLoss?NG=" & NG & "&Good=" & Good & "&Timeshift=" & Timeshift & "&std_cd=" & std_cd)
            Dim jsSerializer As New JavaScriptSerializer()
            ' Deserialize the JSON string to a Dictionary
            Dim data As Dictionary(Of String, Object) = jsSerializer.Deserialize(Of Dictionary(Of String, Object))(jsonString)
            ' Access the value
    ...
    Public Shared Function OEE_getWorkingTime(line_cd As String, Timeshift As String)
        Try
            ...
            Dim jsonString = api.Load_dataOEE("http://" & Backoffice_model.svOEE & "/api/dataGetWorkingTime?line_cd=" & line_cd & "&st_shift=" & Timeshift & "&date_crr=" & date_end & "&time_crr=" & time & "&dates_start=" & convertDateStart)
            Dim jsSerializer As New JavaScriptSerializer()
            ' Deserialize the JSON string to a Dictionary
            Dim data As Dictionary(Of String, Object) = jsSerializer.Deserialize(Of Dictionary(Of String, Object))(jsonString)
            ' Access the value
...
ReConnect:
            ...
            Dim jsonString = api.Load_dataOEE("http://" & Backoffice_model.svOEE & "/api/dataDataTimestart?st_shift=" & st_shift & "&line_cd=" & line_cd & "&date_start=" & convertDateStart & "&dateCurr=" & date_end & "&TimeCurr=" & time)
            Dim jsSerializer As New JavaScriptSerializer()
            Dim data As Dictionary(Of String, Object) = jsSerializer.Deserialize(Of Dictionary(Of String, Object))(jsonString)
...
ReConnect:
            ...
            Dim jsonString = api.Load_dataOEE("http://" & Backoffice_model.svOEE & "/api/dataGetWorkingTimeModel?st_shift=" & st_shift & "&line_cd=" & line_cd & "&dates_start=" & convertDateStart & "&date_crr=" & date_end & "&time_crr=" & time & "&item_cd=" & item_cd)
            Dim jsSerializer As New JavaScriptSerializer()
            Dim data As Dictionary(Of String, Object) = jsSerializer.Deserialize(Of Dictionary(Of String, Object))(jsonString)
    ...
    Public Shared Function OEE_getDataProductionActual(st_shift As String, line_cd As String, item_cd As String) As String
        Try
            ...
            Dim jsonString As String = api.Load_dataOEE("http://" & Backoffice_model.svOEE & "/api/dataDataProductionActual?st_shift=" & st_shift & "&line_cd=" & line_cd & "&dates_start=" & date_st & "&date_crr=" & date_end & "&time_crr=" & time & "&item_cd=" & item_cd)
