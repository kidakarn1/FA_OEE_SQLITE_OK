...
Public Class closeLotsummary
    ...
    Private Const CloseLotNetworkTimeoutSeconds As Integer = 15
    Dim ctsDelay As New Threading.CancellationTokenSource()
    Private netCheckRunning As Integer = 0
    ...
    Public Function calGoodqty(act As Integer, nc As Integer, ng As Integer)
        ...
        Return result
    End Function
    Public Async Function WaitForNetworkWithPopup() As Task(Of Boolean)
        Dim startedAt As DateTime = DateTime.UtcNow
        Do
            Dim networkReady As Boolean = False
            Try
                networkReady = IsNetworkAvailable() AndAlso My.Computer.Network.Ping(Backoffice_model.svp_ping)
            Catch
                networkReady = False
            End Try
            ...
            If (DateTime.UtcNow - startedAt).TotalSeconds >= CloseLotNetworkTimeoutSeconds Then
                If load_show.Visible Then
                    load_show.Hide()
                End If
                Return False
    ...
    Private Async Function TransferBeforeCloseLotAsync() As Task
        Dim transferTask As Task = Backoffice_model.updated_data_to_dbsvr(Me, "1")
        Dim timeoutTask As Task = Task.Delay(TimeSpan.FromSeconds(CloseLotNetworkTimeoutSeconds))
        Dim completedTask As Task = Await Task.WhenAny(transferTask, timeoutTask)
        If completedTask Is transferTask Then
            Await transferTask
        Else
            Console.WriteLine("Close Lot transfer timed out; continuing with the current lot.")
        End If
    End Function
    ...
    Public Async Function Manage_closelot() As Task
        'Try
        If Await WaitForNetworkWithPopup() Then
            ...
            End If
            Await WaitForNetworkWithPopup()
            Await insertProductionactual(sWi, sLine, sPart, pQty, seqQty, sSeq, sShift, staffNo, stDatetime, eDatetime, sLot, cFlg, trFlg, dFlg, prdFlg, clFlg, avarage_eff, avarage_act_prd_time)
            Await WaitForNetworkWithPopup()
        ...
        Else
            btnOk.Enabled = True
            MessageBox.Show("Network connection is unavailable. Please check the server and try Close Lot again.", "Close Lot", MessageBoxButtons.OK, MessageBoxIcon.Warning)
        End If
        'Catch ex As Exception
        'load_show.Show()
        ' End Try
    ...
    Private Async Sub btnOk_Click(sender As Object, e As EventArgs) Handles btnOk.Click
        ...
        Try
            If My.Computer.Network.Ping(Backoffice_model.svp_ping) Then
                If StopMenu.Visible Then
                    StopMenu.SatrtWork()
                End If
                Await TransferBeforeCloseLotAsync() ' check ก่อน Close Lot อีกครั้ง
        ...
        Catch ex As Exception
            btnOk.Enabled = True
            MessageBox.Show("Close Lot failed: " & ex.Message, "Close Lot", MessageBoxButtons.OK, MessageBoxIcon.Error)
        End Try
    End Sub
    ...
    Public Sub checkPrintnormal()
        ...
        ' End If
    End Sub
    Public Async Function insertProductionactual(wi_plan As String, line_cd As String, item_cd As String, plan_qty As String, act_qty As String, seq_no As String, shift_prd As String, staff_no As String, prd_st_datetime As String, prd_end_datetime As String, lot_no As String, comp_flg2 As String, transfer_flg As String, del_flg As String, prd_flg As String, close_lot_flg As String, avarage_eff As String, avarage_act_prd_time As String) As Task
        Try
            If My.Computer.Network.Ping(Backoffice_model.svp_ping) Then
                transfer_flg = "1"
                If MainFrm.chk_spec_line = "2" Then
                    ...
                    For Each itemPlanData As DataPlan In Confrime_work_production.ArrayDataPlan
                        ...
                        Await WaitForNetworkWithPopup()
                        Dim specialServerInserted As Boolean = Backoffice_model.Insert_prd_close_lot(special_wi, line_cd, special_item_cd, plan_qty, act_qty, Iseq, shift_prd, staff_no, prd_st_datetime, prd_end_datetime, lot_no, comp_flg2, transfer_flg, del_flg, prd_flg, close_lot_flg, avarage_eff, avarage_act_prd_time)
                        Dim specialLocalTransferFlg As String = If(specialServerInserted, "1", "0")
                        Backoffice_model.Insert_prd_close_lot_sqlite(special_wi, line_cd, special_item_cd, plan_qty, act_qty, Iseq, shift_prd, staff_no, prd_st_datetime, prd_end_datetime, lot_no, comp_flg2, specialLocalTransferFlg, del_flg, prd_flg, close_lot_flg, avarage_eff, avarage_act_prd_time)
                ...
                Else
                    ...
                    Dim normalServerInserted As Boolean = Backoffice_model.Insert_prd_close_lot(wi_plan, line_cd, item_cd, plan_qty, act_qty, seq_no, shift_prd, staff_no, prd_st_datetime, prd_end_datetime, lot_no, comp_flg2, transfer_flg, del_flg, prd_flg, close_lot_flg, avarage_eff, avarage_act_prd_time)
                    Dim normalLocalTransferFlg As String = If(normalServerInserted, "1", "0")
                    Backoffice_model.Insert_prd_close_lot_sqlite(wi_plan, line_cd, item_cd, plan_qty, act_qty, seq_no, shift_prd, staff_no, prd_st_datetime, prd_end_datetime, lot_no, comp_flg2, normalLocalTransferFlg, del_flg, prd_flg, close_lot_flg, avarage_eff, avarage_act_prd_time)
                    Dim temp_co_emp As Integer = List_Emp.ListView1.Items.Count
                    Dim emp_cd As String
                    For i = 0 To temp_co_emp - 1
                        emp_cd = List_Emp.ListView1.Items(i).Text
...
recheck_defect:
    ...
    Async Function checkNetColselot() As Task
        ' Keep one cancellable polling loop. The previous recursive ContinueWith
        ' created a new task every cycle and could keep the form alive after close.
        If Threading.Interlocked.Exchange(netCheckRunning, 1) <> 0 Then Return

        Try
            Do While Not ctsDelay.IsCancellationRequested AndAlso Not Me.IsDisposed
                Await Task.Delay(3000, ctsDelay.Token)
                If ctsDelay.IsCancellationRequested OrElse Me.IsDisposed OrElse Not Me.IsHandleCreated Then Exit Do

                Dim isOnline As Boolean = Await Task.Run(Function()
                                                             Try
                                                                 Return My.Computer.Network.Ping(Backoffice_model.svp_ping)
                                                             Catch
                                                                 Return False
                                                             End Try
                                                         End Function)
                ...
                Try
                    Me.BeginInvoke(New Action(Sub()
                                                  ...
                                                  loadNet.Visible = Not isOnline
                                                  NextProcess = If(isOnline, 1, 0)
                                              End Sub))
                Catch
                    Exit Do
                End Try
        ...
        Catch ex As Threading.Tasks.TaskCanceledException
            ' Expected when the summary form is closing.
        Catch
            ' Do not let a network error fault the UI task.
        Finally
            Threading.Interlocked.Exchange(netCheckRunning, 0)
        End Try
    End Function

    Private Sub closeLotsummary_FormClosing(sender As Object, e As FormClosingEventArgs) Handles MyBase.FormClosing
        Try
            ctsDelay.Cancel()
            ctsDelay.Dispose()
        Catch
